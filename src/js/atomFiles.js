(function ($) {
  $.fn.atomFileInput = function () {
      ...
      $input.on("change", function (element) {
        ...
        if (fileName) {
          ...
          $label
            .addClass("has-file")
            .find("i")
            .removeClass("fas fa-cloud-upload-alt")
            .addClass("far fa-file-alt");
        } else {
    ...
    if (this.length > 1) {
      var collection = [];
      this.each(function () {
        collection.push($(this).atomFileInput());
      });
      return collection;
    } else {
  ...
  $.fn.atomFileLoader = function () {
    this.initialize = function () {
      var uniqID =
        Date.now() * Math.round(Math.random() * Math.random() * 1000);
      ...
      uploader.change(function () {
        ...
        if (filename) {
          ...
          blocResult
            .find(".file-recap")
            .html("Fichier : " + filename[filename.length - 1]);
          blocResult.show();
        } else {
          $(this).find("input").val("");
        }
    ...
    if (this.length > 1) {
      var collection = [];
      this.each(function () {
        collection.push($(this).atomFileLoader());
      });
      return collection;
    } else {
      return this.initialize();
