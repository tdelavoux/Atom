...
$(function () {
  ...
  async function compute(form, self) {
    var blocage = false;
    var notifyNone =
      form.attr("a-notify-none") !== undefined &&
      form.attr("a-notify-none") !== "false"
        ? true
        : false;
    var notifyAll =
      form.attr("a-notify-all") !== undefined &&
      form.attr("a-notify-all") !== "false"
        ? true
        : false;
    var notifyInputNone =
      form.attr("a-input-notify-none") !== undefined &&
      form.attr("a-input-notify-none") !== "false"
        ? true
        : false;
    var colorInput =
      form.attr("a-disable-color") !== undefined &&
      form.attr("a-disable-color") !== "false"
        ? true
        : false;
    var checkVisi = form.attr("a-check-invisible")
      ? form.attr("a-check-invisible")
      : false;
    var optVerif = form.attr("a-data-opt-bloc")
      ? form.attr("a-data-opt-bloc")
      : false;
    var callback = form.attr("a-data-function")
      ? form.attr("a-data-function")
      : false;
    var err = new Array();
    ...
    $(".a-verify-select", form).each(function () {
      ...
      var errCustomMsg = $(this).attr("a-error-message")
        ? $(this).attr("a-error-message")
        : "Champs incorrect";
      var displayMsg =
        $(this).attr("a-disable-message") !== undefined &&
        $(this).attr("a-disable-message") !== "false"
          ? false
          : true;
      ...
      if (checkVisi || $(this).is(":visible")) {
        if (
          ...
          $("option:selected", this).val() === verif
        ) {
          displayMsg &&
            err.push({ elem: $(this), errorMessage: error, name: name });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
          blocage = true;
        } else if (
          $(this).val() !== E_S &&
          eval($(this).attr("a-alternate-verif"))
        ) {
          displayMsg &&
            err.push({ elem: $(this), errorMessage: errCustomMsg, name: name });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
          blocage = true;
    ...
    $(".a-verify-text", form).each(function () {
      var minLength = $(this).attr("a-min-length")
        ? parseInt($(this).attr("a-min-length"))
        : 0;
      var maxLength = $(this).attr("a-max-length")
        ? parseInt($(this).attr("a-max-length"))
        : null;
      var errCustomMsg = $(this).attr("a-error-message")
        ? $(this).attr("a-error-message")
        : "Champs incorrect";
      ...
      var nullable =
        $(this).attr("a-nullable") !== undefined &&
        $(this).attr("a-nullable") !== "false"
          ? true
          : false;
      var displayMsg =
        $(this).attr("a-disable-message") !== undefined &&
      ...
      if (checkVisi || $(this).is(":visible")) {
        if ($(this).val().trim() === E_S && !nullable) {
          displayMsg &&
            err.push({
              elem: $(this),
              errorMessage: "Saisie obligatoire",
              name: name,
            });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
          ...
          displayMsg &&
            err.push({
              elem: $(this),
              errorMessage:
                "Saisie trop courte (min " + minLength + " caractères)",
              name: name,
            });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
          ...
          displayMsg &&
            err.push({
              elem: $(this),
              errorMessage:
                "Saisie  trop longue (max " + maxLength + " caractères)",
              name: name,
            });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
    ...
    $(".a-verify-date", form).each(function () {
      var format = $(this).attr("a-date-format")
        ? $(this).attr("a-date-format").toUpperCase()
        : "DD/MM/YYYY";
      var nullable =
        $(this).attr("a-nullable") !== undefined &&
        $(this).attr("a-nullable") !== "false"
      ...
      if (checkVisi || $(this).is(":visible")) {
        if ($(this).val().trim() === E_S && !nullable) {
          ...
          displayMsg &&
            err.push({
              elem: $(this),
              errorMessage: "Format invalide. Attendu : " + format,
              name: name,
            });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
    ...
    $(".a-verify-phone", form).each(function () {
      var errCustomMsg = $(this).attr("a-error-message")
        ...
        : "Champs incorrect";
      var format =
        $(this).attr("a-phone-format") &&
        $(this).attr("a-phone-format") === "mobile"
          ? "mobile"
          : "fixe";
      ...
      // Formatage de la REGEX a partir du format
      var regFormatFixe =
        "([0][0-9]{9})$|([0][0-9])(([ ][0-9]{2}){4})$|([0][0-9])(([.][0-9]{2}){4})";
      var regFormatMobile =
        "([06][0-9]{9})$|([06])(([ ][0-9]{2}){4})$|([06])(([.][0-9]{2}){4})";
      var regFormat =
        "^" + (format === "mobile" ? regFormatMobile : regFormatFixe) + "$";
      var regex = new RegExp(regFormat);
        ...
        if ($(this).val().trim() === E_S && !nullable) {
          ...
          displayMsg &&
            err.push({
              elem: $(this),
              errorMessage:
                "Format inconnu. sans séparateur, point ou espace acceptés.",
              name: name,
            });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
    ...
    $(".a-verify-int", form).each(function () {
      var minVal = parseInt($(this).attr("a-min"))
        ? parseInt($(this).attr("a-min"))
        : 0;
      var maxVal = parseInt($(this).attr("a-max"))
        ? parseInt($(this).attr("a-max"))
        : 2147483647;
      var name = $(this).attr("a-name") ? $(this).attr("a-name") : "Mail";
      var errCustomMsg = $(this).attr("a-error-message")
        ? $(this).attr("a-error-message")
      ...
      if (checkVisi || $(this).is(":visible")) {
        if (
          $(this).val() &&
          (!$.isNumeric($(this).val()) || $(this).val().includes(".") !== false)
        ) {
          displayMsg &&
            err.push({
              elem: $(this),
              errorMessage: "Nombre entier attendu",
              name: name,
            });
          $(this).parent().find(".a-input-error-text").detach();
          !colorInput && $(this).parent().addClass("a-form-error");
    ...
    $(".a-verify-float", form).each(function () {
      ...
      if (checkVisi || $(this).is(":visible")) {
        if ($(this).val() && !$.isNumeric($(this).val())) {
          ...
          parseFloat($(this).val()) < minVal ||
          parseFloat($(this).val()) > maxVal
        ) {
          displayMsg &&
            err.push({
              elem: $(this),
    ...
    $(".a-verify-checked", form).each(function () {
      var error =
        "Attention, " +
        ($(this).attr("a-name") ? $(this).attr("a-name") : "La checkbox ") +
        " n'a pas été accepté(e) / coché(e). ";
    ...
    if (blocage === true) {
        ...
        err.forEach(function (element) {
          if (element.elem.parent().hasClass("a-count-area")) {
            element.elem
              .parent()
              .append(
                '<span class="a-input-error-text-with-count">' +
      ...
      var errMsg = [
        ...new Map(err.map((item) => [item["name"], item])).values(),
      ];

      if (notifyAll) {
        errMsg.forEach(function (element) {
          $.atomNotification(
            (element.name ? element.name : "Champ incorrect ") +
              " : " +
              element.errorMessage,
            "danger"
          );
